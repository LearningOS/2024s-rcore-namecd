...
impl TaskManager {
    ...
    fn run_first_task(&self) -> ! {
        ...
        task0.task_begin_time = get_time_ms();
    ...
    fn run_next_task(&self) {
        if let Some(next) = self.find_next_task() {
            ...
            if inner.tasks[next].task_begin_time == 0 {
                inner.tasks[next].task_begin_time = get_time_ms();
            }
    ...
    pub fn get_task_info(&self) -> (TaskStatus, [u32; MAX_SYSCALL_NUM], usize) {
        ...
        (task.task_status, task.task_syscall_times, get_time_ms() - task.task_begin_time)
    }
}
...
/// 受到作业讲解的启发，在这里使用increase_syscall_times函数来增加系统调用次数，通过syscall调用这个函数
/// 来增加系统调用次数
/// increase the syscall times of current task
